...
def draw_text(display, text, size, x, y, color=(255, 255, 255)):
    ...
    Parameters:
        display (pygame.Surface): The surface to draw the text on.
...
def show_gameover_screen():
    ...
    draw_text(gameDisplay, "Powered by :=> Script Squad", 40, WIDTH / 6, HEIGHT / 15)
    # draw_text(gameDisplay, "Bano Qabil", 90, WIDTH / 6, HEIGHT / 12, GREEN)
    if not game_over :
        draw_text(gameDisplay,"Score : " + str(score), 50, WIDTH / 2, HEIGHT /2)

    draw_text(gameDisplay, "Press a key to Start Game!", 64, WIDTH / 2, HEIGHT * 3 / 4)
    pygame.display.flip()
    waiting = True
    while waiting:
        clock.tick(FPS)
